...
from common import load_data, number, parser, percent_change, price, records, write_output
...
def calculate_breadth(rows: list[dict[str, Any]], group_by: str = "market") -> dict[str, Any]:
    ...
    for group, members in sorted(grouped.items()):
        changes = [percent_change(price(r), r.get("prev_close")) for r in members]
        ...
        dma_pairs = [(price(r), number(r.get("dma20"))) for r in members]
        dma_pairs = [(last, dma) for last, dma in dma_pairs if last is not None and dma is not None]
        highs = sum(1 for r in members if price(r) is not None and number(r.get("high_52w")) is not None and price(r) >= number(r.get("high_52w")))
        lows = sum(1 for r in members if price(r) is not None and number(r.get("low_52w")) is not None and price(r) <= number(r.get("low_52w")))
