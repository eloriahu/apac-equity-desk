...
Optional claim fields: `value`, `unit`, `currency`, `session`, `tolerance`, `fact_key`, `max_age_minutes`, `new_today`, `confirmed`, `movement`, `driver_status` and `mechanism`. Set `movement: true` for a material index, sector, theme or security move described in reader-facing prose. Such claims require `driver_status: confirmed|plausible|unresolved` plus a nonempty `mechanism`; for an unresolved move, use `mechanism` to record why the ranked candidates do not settle the cause. The checker flags missing citations, causal claims supported only by Level 4 or by sources with no `evidence_level`, material moves without a driver assessment or mechanism, malformed or future source timestamps, evidence older than a claim's explicit `max_age_minutes`, contradictory numeric claims sharing the same `fact_key`, and repeated facts that mix units, currencies or sessions. Set freshness limits only for genuinely live claims; a filing or structural background item is not stale merely because it is old. Source independence, semantic support and whether an event caused a move still require analyst verification.
